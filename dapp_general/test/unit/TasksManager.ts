...
describe("TasksManager Unit Tests", function () {
  ...
  describe("receiveResults", function () {
    beforeEach(async function () {
      tasksManager = tasksManagerContract.connect(client);
      await tasksManager.createTask(
        taskID,
    ...
    it("reverts if not called by the provider", async function () {
      await expect(
        tasksManager.receiveResults(taskID, "ipfsCID")
    ...
    it("receiveResults is successful, cost is lower than clientCollateral", async function () {
      //checks task state
      ...
      //emits a TaskReceivedResults event
      const duration = 2;
      tasksManager = tasksManagerContract.connect(provider);
      await tasksManager.activateTask(taskID, {
        value: providerCollateralValue,
      ...
      expect(taskState).to.equal("ResultsReceived");
      ...
      expect(events[4].event).to.equal("TaskReceivedResults");
      expect(events[4].args.taskID).to.equal(taskID);
    });

    it("receiveResults is successful, cost is greater than clientCollateral", async function () {
      //checks task state
      //checks payment state
      //checks upVotes
      //emits a TransferMadeToProvider event
      ...
      expect(events[2].event).to.equal("ProviderUpvoted");
      expect(events[2].args.provider).to.equal(provider.address);
      expect(events[2].args.taskID).to.equal(taskID);
      expect(events[3].event).to.equal("TaskReceivedResults");
      expect(events[3].args.taskID).to.equal(taskID);
    });

    it("receiveResults is unsuccessful", async function () {
      //checks task state
      //checks payment state
      //checks downVotes
      //emits a TransferMadeToClient event
      ...
      expect(events[1].args.taskID).to.equal(taskID);
      expect(events[2].event).to.equal("TaskReceivedResults");
      expect(events[2].args.taskID).to.equal(taskID);
    });
  });
  ...
  describe("completePayment", function () {
    ...
    it("reverts if task is not in state ResultsReceived", async function () {
      tasksManager = tasksManagerContract.connect(client);
      await expect(tasksManager.completePayment(taskID)).to.be.revertedWith(
        "Invalid TaskState."
      );

...
function GetBids() {
  ...
  const finalizeBtnHandler = async () => {
    ...
    const valueEth = ethers.utils.parseEther(
      (clientCollateral / wei).toFixed(18).toString()
    );
    await auctionContract.finalize(auctionID, selectedProvider, {
      value: valueEth,
    });
  };
      ...
      const mappedAuctions = auctions.map((auction) => {
        return auction.auctionState === 0 &&
          ...
          ? {
              ...
              auctionID:
                auction.auctionID.slice(0, 6) +
                "..." +
                auction.auctionID.slice(-4),
              creationTime: new Date(
                auction.creationTime.toNumber() * 1000
              ).toLocaleString("en-GB"),
              auctionDeadline: new Date(
                (auction.creationTime.toNumber() +
                  auction.auctionDeadline.toNumber()) *
                  1000
              ).toLocaleString("en-GB"),
  ...
  return (
   ...
   have stored in usestate */}
      ...
      <Card className="text-center">
        {bidsSelected && bids ? (
          <CTable columns={bidsColumns} items={mapBids(bids)} />
        ) : null}
      </Card>
      <Card className="text-center">
        {bidsSelected && bids ? (

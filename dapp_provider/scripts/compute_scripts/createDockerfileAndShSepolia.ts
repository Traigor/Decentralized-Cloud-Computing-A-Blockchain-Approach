...
export async function createDockerfileAndShSepolia({
  taskID,
  privateKey,
}: TCreateDockerfile) {
...
COPY yarn.lock .
COPY tsconfig.json .
COPY TasksManagerSepolia.json TasksManager.json

RUN yarn &&\
 yarn hardhat run scripts/completeTask.ts --network sepolia  
